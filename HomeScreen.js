...
export default function HomeScreen() {
  ...
  const handleEmployeeLoginPress = () => {
    if (isEmpLoggedIn) {
      AsyncStorage.removeItem('loggedInEmployee');
      setEmpIsLoggedIn(false);
    } else {
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={{ paddingBottom: 100 }}>
            ...
            <View>
              ...
              {showDropdown && (
                <View style={styles.dropdownMenu}>
                    ...
                    <TouchableOpacity style={styles.dropdownItem} onPress={() => navigation.navigate('MyBookings')}>
        ...
        <FlatList
         ...
         renderItem={({ item }) => (
          <TouchableOpacity
           onPress={() => navigation.navigate(item.name)} // screen name matches item.name
  ...
  renderItem={({ item }) => (
    <TouchableOpacity
      onPress={() => navigation.navigate(item.title)} // title must match registered screen name
