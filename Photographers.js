...
const PhotographersScreen = ({ navigation }) => {
  ...
  const fetchPhotographers = async () => {
    try {
      const res = await axios.get(`${apiurl}/api/authRoutes/employeesByField`, {
        params: { fieldOfWork: 'Photographers' },
      });
      if (res.data.success) {
        setPhotographers(res.data.employees);
      } else {
        setPhotographers([]);
      }
    } catch (error) {
  ...
  const openBookingForm = (photographer) => {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.body}>
        {loading ? (
          ...
          <Text style={styles.bodyText}>No photographers found.</Text>
        ) : (
          <FlatList
            data={photographers}
      ...
      <BookingModal
        ...
        selectedEmployee={selectedPhotographer}
